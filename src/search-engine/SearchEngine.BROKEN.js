 ...
 * @property {number} [maxResults=100] - Maximum number of results
 * @property {number} [minRelevance=0.01] - Minimum relevance score (0-1)
...
export class SearchEngine {
    constructor(database = null, options = {}) {
        console.log('🔧 SearchEngine constructor called with:', {
            databaseType: typeof database,
            databaseConstructor: database?.constructor?.name,
            hasArticles: !!database?.articles,
            articleCount: database?.articles?.size
        });
        
        // Handle both parameter styles:
        // new SearchEngine(database, options) OR new SearchEngine({database, ...options})
        if (database && typeof database === 'object' && 
            !database.articles && // If it doesn't have articles, it's probably options
            (database.constructor.name === 'Object' || database.hasOwnProperty('database'))) {
            // First parameter is options object
            console.log('🔧 Treating first parameter as options object');
            options = database;
            this.database = options.database || null;
        } else {
            // First parameter is database
            console.log('🔧 Treating first parameter as database');
            this.database = database;
        }
        
        console.log('🔧 Final database assignment:', {
            hasDatabase: !!this.database,
            databaseType: typeof this.database,
            articleCount: this.database?.articles?.size
        });
        
        this.options = {
            maxResults: 50,
            minRelevance: 0.001,  // Reduced minimum relevance to show more results
            fuzzyThreshold: 0.6,  // Reduced fuzzy threshold for more flexible matching
            enableBooleanSearch: true,
            enablePhraseSearch: true,
            enableWildcardSearch: true,
            enableFuzzySearch: true,
            defaultOperator: 'OR',  // Changed from AND to OR for broader results
            ...options
        };
        ...
        // Auto-initialize if database is provided
        if (this.database) {
    ...
    /**
     * Initialize the search engine with database
    ...
    async initialize() {
        ...
        if (this.database.articles) {
            // LegalDatabase format
            articles = Array.from(this.database.articles.values());
        } else if (this.database.getArticles) {
            // Enhanced database format
            articles = await this.database.getArticles();
        } else {
            throw new Error('Database does not have articles or getArticles method');
        }
        
        console.log(`Found ${articles.length} articles to index`);
        
        // Build search index
        await this.buildIndex(articles);
    ...
    async executeSearch(query, options) {
        ...
        const results = [];
        console.log(`Processing ${candidateArticles.size} candidate articles...`);
        
        for (const articleId of candidateArticles) {
            const metadata = this.documentMetadata.get(articleId);
            if (!metadata) {
                console.warn(`No metadata found for article ${articleId}`);
                continue;
            }
            
            const relevanceScore = this.calculateRelevance(articleId, query, termScores, options);
            console.log(`Article ${articleId}: relevance=${relevanceScore}, threshold=${options.minRelevance}`);
            
            if (relevanceScore >= options.minRelevance) {
                try {
                    // Try to get the full article from the database
                    let article = null;
                    
                    if (this.database && this.database.articles) {
                        // Get article directly from the database articles Map
                        const articleData = this.database.articles.get(articleId);
                        if (articleData) {
                            // Create a proper article object with safe defaults
                            article = {
                                id: articleData.id || articleId,
                                number: articleData.number || articleData.id || articleId,
                                title: articleData.title || articleData.content?.substring(0, 100) || `Article ${articleId}`,
                                content: articleData.content || articleData.title || '',
                                category: articleData.category || 'general',
                                translations: articleData.translations || {},
                                lastModified: articleData.lastModified || new Date().toISOString(),
                                ...articleData // Preserve all original properties
                            };
                        }
                    }
                    
                    if (!article) {
                        console.warn(`Article not found for ID: ${articleId}. Available articles: ${this.database.articles.size}`);
                        // Try to get any article as fallback
                        const fallbackArticle = Array.from(this.database.articles.values())[0];
                        if (fallbackArticle) {
                            console.log('Using fallback article structure:', Object.keys(fallbackArticle));
                        }
                        continue;
                    }
                    ...
                    console.log(`Created result for ${articleId}:`, {
                        id: searchResult.id,
                        title: searchResult.article.title?.substring(0, 50),
                        relevance: searchResult.relevanceScore
                    });
        ...
        console.log(`Returning ${Math.min(results.length, options.maxResults)} results`);
    ...
    calculateRelevance(articleId, query, termScores, options) {
        ...
        // Simple scoring: if any term matches, give a decent score
        ...
        // Simple scoring: base score for any match, bonus for multiple matches
        if (matchCount === 0) return 0;
        
        let score = 0.1 + (matchCount * 0.2); // Base 0.1 + 0.2 per match
        
        // Bonus for title matches (check if title contains search terms)
        if (metadata.title) {
        ...
        return Math.min(1.0, Math.max(0.02, score)); // Ensure minimum 0.02 score
    }
